...
{
	return PXI8265_MAX_CHAN_AO * PXI8265_CARD_COUNT;
}
...
BOOL CPXI8265_AO::Start(ULONG ulChan)
{
	...
	//for (int chan=0; chan<PXI8265_MAX_CHAN_AO; chan++)
	{
		...
		result = PXI8265_AO_InitChan(m_hDevice,ulChan,PXI8265_VAL_MIN,PXI8265_VAL_MAX);
...
BOOL CPXI8265_AO::WriteAO(ULONG ulChan, double dfVolt)
{
	...
	if ((dfVolt>PXI8265_VAL_MAX) || (dfVolt<PXI8265_VAL_MIN))
	{
		_WriteLog("WriteDC 输出超限(范围%.0f~%.0f) 当前%.3f", PXI8265_VAL_MIN, PXI8265_VAL_MAX, dfVolt);
		dfVolt = LIMIT_NUM(dfVolt, PXI8265_VAL_MIN, PXI8265_VAL_MAX);
	}

	// 获得卡号
	ULONG cardNum = ulChan / PXI8265_MAX_CHAN_AO;
	// 获得卡对应的通道号
	ULONG cardChan = ulChan % PXI8265_MAX_CHAN_AO;
