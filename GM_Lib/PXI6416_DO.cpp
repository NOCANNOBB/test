...
BOOL PXI6416_DO::WriteDO(ULONG ulChan, byte boolValue)
{
	...
	PDEV_DO			psDevAO;
	ULONG cardNum = ulChan / PXI6416_CARD_COUNT;
	ULONG chanNum = ulChan % PXI6416_MAX_CHAN_DO;
	psDevAO = &m_sDevAO[cardNum];

	psDevAO->arrayDO[chanNum] = boolValue != 0 ? TRUE : FALSE;

	BOOL bRet = _IsErrChk(PXI6416_DO_Write(psDevAO->hDev, psDevAO->arrayDO, PXI6416_MAX_CHAN_DO), 0);
